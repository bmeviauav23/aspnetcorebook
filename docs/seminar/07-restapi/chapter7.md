...
A következő dialógusablakban válasszuk ki a *.NET 10* opciót. Az extrák közül **ne** kérjük ezeket: HTTPS, Docker, authentikáció. Viszont hagyjuk bepipálva a Controller és az OpenAPI támogatást.
A generált projektből törölhetjük a minta API fájljait, azaz a `Weather` kezdetű fájlokat a projekt gyökeréből és a `Controllers` mappából.

Adjuk hozzá függőségként:

- a *BLL* projektet (*Api projekt Dependencies jobb gomb / Add Project Reference…*)
...
!!! warning Microsoft csomagok verziózása
    ...
    Ha a projektünk például .NET 10-es keretrendszert használ, akkor az Entity Framework Core és egyéb extra ASP.NET Core csomagok közül is olyan verziót válasszunk, ahol legalább a főverzió egyezik, tehát valamilyen 10.x verziót. Ez nem azt jelenti, hogy az inkonzisztens verziók mindig hibát eredményeznek, inkább a projekt általában stabilabb, ha a főverziók közötti váltást egyszerre, külön migrációs folyamat ([példa](https://learn.microsoft.com/en-us/aspnet/core/migration/31-to-60)) keretében végezzük.
...
Az Entity Framework gyakorlat alapján hozzunk létre egy új LocalDB adatbázist egy választott névvel, pl. neptun kód. Az SQL Server Object Explorer-ből a connection string-et lopjuk el. (*nyissuk le az adatbáziskapcsolatot / jobbklikk az adatbázison / Properties / a Properties ablakból a Connection String értéke*).
...
Fordítsuk a teljes solution-t, állítsuk be indítandó (startup) projektnek az új Web API projektet (*jobbklikk a projekten / Set as Startup Project*).
Nyissunk egy terminált a solution gyökerében, és navigáljunk el a `WebApiLab.DAL` könyvtárba, majd készítsünk egy migrációt és futtassuk le.
Szükséges megadni a `-s` kapcsolóval az API projekt helyét, hogy a migrációs eszköz a futtatható projektből kiolvashassa a connection stringet.

``` cmd
dotnet ef migrations add Init -s ..\WebApiLab.Api\
dotnet ef database update -s ..\WebApiLab.Api\
```
...
!!! warning "Projektek a migrációhoz"
    ...
    Itt mutatkozik meg, hogy a migráció lényegében egy teljes alkalmazásindítást jelent a `Program` osztályon keresztül: inicializálódik a DI konténer, a konfigurációs objektum stb.
...
Adjunk hozzá egy új Controllert a *Controllers* mappába (*Add / Controller / bal fában Common / API / jobb oldalon API Controller with read/write actions*) `EFProductController` néven.
...
A swagger használatához szükséges még egy swagger ui implementációt behúznunk a projektbe, mivel jelenleg csak az openapi leíró publikálása van beállítva alapértelmezetten a projektben. Legyen ez a csomag az `NSwag.AspNetCore`:

```xml
<PackageReference Include="NSwag.AspNetCore" Version="14.7.1" />
```

A swagger ui-t pedig egy plusz middleware segítségével engedélyezhetjük, de előtte sajnos vissza kell állnunk 3.0-as verzióra, mert a 3.1-es verzióban egyelőre nem működik a swagger ui megjelenítése.
...
Ehhez a megoldáshoz készítsünk külön kontroller változatot. A *Controllers* mappába hozzunk létre egy kontrollert (*Add / Controller / bal fában Common / API / jobb oldalon API Controller with read/write actions*) `ProductController` néven.
...
Írjunk új `Get()` változatot az eredeti helyett a `ProductController`-be az `IProductService` függőséget felhasználva:
...
A leképezési konfigurációkat profilokba szervezve adhatjuk meg. Adjunk hozzá a BLL projekthez egy új osztályt `MappingProfile` néven a *Dtos* mappába.
...
Az AutoMapper-t konfigurálhatjuk az átadott lambda kifejezéssel vagy megadhatunk assemblyket, amikből automatikusan beregisztrákja a profilokat.
...
A `ProductController`-ben, az `IProductService`-ben és a `ProductService`-ben az entitásokra mutató névteret cseréljük ki a DTO-kra mutatóra:
...
Írjuk át a product service-ben a `GetProducts` függvényt, hogy a DTO-kat adja vissza.
...
### ProjectTo használata

Van lehetőségünk AutoMapperrel az SQL lekérdezésbe is belefordítani a mappelési logikát, ez a `ProjectTo` függvény használatával érhető el.
...
{
    var efProduct = mapper.Map<Dal.Entities.Product>(newProduct);
...
public Product UpdateProduct(int productId, Product updatedProduct)
{
    var efProduct = context.Products.SingleOrDefault(p => p.Id == productId)
        ?? throw new EntityNotFoundException("Nem található a termék", productId);
    mapper.Map(updatedProduct, efProduct);
    context.SaveChanges();
    return GetProduct(efProduct.Id);
}        
```
...
public void DeleteProduct(int productId)
{
    var efProduct = context.Products.SingleOrDefault(p => p.Id == productId)
        ?? throw new EntityNotFoundException("Nem található a termék", productId);
    context.Products.Remove(efProduct);
    context.SaveChanges();
}
```
...
!!! tip "Törlés lekérdezés nélkül"
    ...
    public void DeleteProduct(int productId)
    {
        context.Products.Remove(new Dal.Entities.Product(null!) { Id = productId });
        context.SaveChanges();
    }
    ```
...
A jelenlegi ajánlás ehhez az `ActionResult<T>` használata. Elég `T`-t visszaadnunk a függvényben, automatikusan `ActionResult<T>` típussá konvertálódik.
...
!!! tip "DTO-k létrehozása művelet típusonként"
    Gyakori, hogy külön DTO-kat hozunk létre művelet típusonként, pl. `CreateProductRequest`, `UpdateProductRequest` stb., amik a műveletek igényei szerint tartalmazzák az adatokat.
